...
`GET http://localhost:8000/health` returns `{"status": "ok"}`. Run tests: `pytest` from `backend/` — 72 tests: the customer-initiated payment flow end-to-end (register → connect providers → route a payment → fall back on decline → idempotent replay → transaction history), the merchant-initiated flow now authenticated as the merchant and the device throughout (register/login → register a device → open a request → claim it → routes the same way → double-claim rejected → cancel, ownership-checked → plus BioFinance ID push pairing: a targeted request only the matching customer can claim, a pending-request listing, rate limiting on repeated targeted requests), merchant register/login/`me` and its token type being rejected by customer endpoints and vice versa (`test_merchant_auth.py`), merchant device registration and its enforcement — an unregistered device, or one registered to a different merchant, gets 403 (`test_merchant_devices.py`, plus more in `test_biopos_payment_flow.py`), the Phase 6 end-to-end demonstration (`test_end_to_end_demo.py`, also runnable standalone — see below), the BioRouter fallback algorithm in isolation, the Daraja provider against a mocked HTTP transport (`respx`), the Daraja callback handler, customer device registration (`POST /devices/register`), the FCM push service (unit tests against a mocked HTTP transport, plus a wiring test confirming `create_payment_request` actually calls it), the sliding-window rate limiter in isolation, and the provider catalog (list, country filter, connect rejecting a code the catalog doesn't know about) — all against the real database except the mocked-transport ones and the pure-Python rate limiter tests.
