...
Phase 8 (audit logging) is built: `audit_events` existed since the very first migration but nothing wrote to it until now — `app/services/audit_service.py` wires the documented minimum event set (logins, device registration, provider connect/disconnect, routing changes, the full payment lifecycle including the async Daraja-callback path, BioID locking) into the operations that already existed, each event staged in the same DB transaction as the thing it's auditing. Not done: `BIOMETRIC_SUCCESS`/`BIOMETRIC_FAILED` (no backend signal exists — biometric auth is entirely client-side, and a client self-report would contradict the security model's own trust boundary), `DEVICE_REMOVED` (no removal endpoint exists), and there's no read endpoint for the trail yet. See [`docs/roadmap.md`](docs/roadmap.md) Phase 8.
...
`GET http://localhost:8000/health` returns `{"status": "ok"}`. Run tests: `pytest` from `backend/` — 82 tests covering, roughly by file: the customer-initiated payment flow end-to-end (`test_payment_flow.py`), the merchant-initiated flow authenticated as both the merchant and the device throughout plus BioFinance ID push pairing and rate limiting (`test_biopos_payment_flow.py`), merchant register/login/`me` and its token type being rejected by customer endpoints and vice versa (`test_merchant_auth.py`), merchant device registration and its enforcement (`test_merchant_devices.py`), the Phase 6 end-to-end demonstration (`test_end_to_end_demo.py`, also runnable standalone — see below), audit logging actually landing rows for the documented event set, not just the endpoints returning the right status (`test_audit_service.py`), the per-transaction amount cap on both payment-creation paths (`test_transaction_limits.py`), the BioRouter fallback algorithm in isolation (`test_router_service.py`), the Daraja provider against a mocked HTTP transport and its callback handler (`test_daraja_provider.py`, `test_daraja_callback.py`), customer device registration (`test_devices.py`), the FCM push service unit + wiring tests (`test_push_service.py`), the sliding-window rate limiter in isolation (`test_rate_limit.py`), and the provider catalog (`test_provider_catalog.py`) — all against the real database except the mocked-HTTP-transport and pure-Python ones.
