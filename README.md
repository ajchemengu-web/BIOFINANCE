...
Phase 5 (BioPOS, `biopos/`) — backend is now fully real: `POST /merchants/register`/`login` issue a merchant-scoped JWT distinct from a customer's, `POST /payments/request` and `POST /payments/{id}/cancel` both require it and derive `merchant_id` from the token rather than trusting the caller. `biopos/` itself is **not yet wired to this** — it still does the old "sign-in creates a fresh `Merchant` row, no real login" thing and would need updating to actually work against the current API (deliberately deferred, no laptop to verify Flutter changes against right now). Also still open: `merchant_devices` enforcement (authenticating *which merchant* is real now; *which device* isn't yet), and, of BioFinance ID push pairing, the Flutter side (receiving the push, a BioFinance-ID-entry screen). The push-pairing backend (targeted requests, the `claim` ownership check, the FCM push send, `GET /payments/pending`, rate limiting) is done. See [`docs/roadmap.md`](docs/roadmap.md) Phase 5.
...
`GET http://localhost:8000/health` returns `{"status": "ok"}`. Run tests: `pytest` from `backend/` — 64 tests: the customer-initiated payment flow end-to-end (register → connect providers → route a payment → fall back on decline → idempotent replay → transaction history), the merchant-initiated flow now authenticated as the merchant throughout (register/login → open a request → claim it → routes the same way → double-claim rejected → cancel, ownership-checked → plus BioFinance ID push pairing: a targeted request only the matching customer can claim, a pending-request listing, rate limiting on repeated targeted requests), merchant register/login/`me` and its token type being rejected by customer endpoints and vice versa (`test_merchant_auth.py`), the BioRouter fallback algorithm in isolation, the Daraja provider against a mocked HTTP transport (`respx`), the Daraja callback handler, device registration (`POST /devices/register`), the FCM push service (unit tests against a mocked HTTP transport, plus a wiring test confirming `create_payment_request` actually calls it), the sliding-window rate limiter in isolation, and the provider catalog (list, country filter, connect rejecting a code the catalog doesn't know about) — all against the real database except the mocked-transport ones and the pure-Python rate limiter tests.
