...
Replaces the open-claim gap above with an STK-Push-style flow, keyed on the BioFinance ID (`bio_ids.code`) instead of a phone number. Concrete flow: `docs/architecture.md`. Full endpoint/schema changes: `docs/api-spec.md`, `docs/database-schema.md`. Built so far: `bio_id_code` resolution on `POST /payments/request` and the ownership check on `claim` (`backend/app/services/payment_service.py`) — ID lookup and the actual authorization gate both work end-to-end. Not yet built: sending the push notification itself (`push_service.py`/FCM) and `GET /payments/pending` as its fallback — until those land, the customer still has to already know the transaction id to claim a targeted request. This section covers the trust boundary the whole design rests on.
...
- **Abuse surface: unsolicited push spam.** Anyone who knows (or brute-forces) a valid BioFinance ID can trigger a push to that person merely by opening a request against it — no payment completes without their device biometric, but it's still an annoyance/harassment vector. Rate-limit `POST /payments/request` per merchant and per targeted `bio_id_code`, and feed repeated unclaimed push-pairing requests against the same `bio_id` into the same suspicious-activity logging already planned under "Fraud protection" below.
