...
| POST | `/payments/request` | merchant-initiated (`biopos/`): open a payment request, unauthenticated (no merchant-auth endpoint exists — see `docs/roadmap.md` Phase 5), requires `Idempotency-Key`. Optional `bio_id_code` — when given, resolves to a `bio_id` and attaches it on the row immediately instead of leaving it null (404 if the code doesn't match anything). Omitted, it's the existing open request with no target customer. | `bio_id_code` resolution: done — push notification to `POST /devices/register`'d devices: planned |
